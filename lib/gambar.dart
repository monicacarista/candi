  ...
  Future<List<Tripleset>> mainSearch() async {
    ...
    if (response.statusCode == 200) {
      ...
      for (var data in head.results.listTriples) {
        print(data);
        Tripleset tp = Tripleset(data.id,data.idasal,data.candi,data.lokasi,data.gambar, data.gambar1,data.gambar2,data.jenis,data.deskripsi,data.arca,data.upacara,data.relief,data.struktur_bangunan,data.bahan,data.namaLain,data.map,data.data);
        //print(data);
        jokes.add(tp);
      }
      // print(jokes);
